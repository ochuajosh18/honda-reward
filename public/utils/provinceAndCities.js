...
const loadProvinces = selectId => {
	const provinces = getProvinces();

	$(`#${selectId}`).empty();
	$(`#${selectId}`).append('<option selected disabled> - Select - </option>');

	provinces.forEach(province => $(`#${selectId}`).append(`<option value='${province}'>${province}</option>`));
}

const loadCities = (province, selectId) => {
	const cities = provincesAndCities[province];

	$(`#${selectId}`).empty();
	$(`#${selectId}`).append('<option selected disabled> - Select - </option>');

	cities.forEach(city => $(`#${selectId}`).append(`<option value='${city}'>${city}</option>`));
}
